...
class Agent(object):
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # START OF YOUR CODE
            # ------------------------------------------------------------------
            standard_deviation = tf.exp(sy_logstd)
            sampled_z = tf.random_normal(tf.shape(sy_mean))
            sy_sampled_ac = sy_mean + standard_deviation * sampled_z
            # ------------------------------------------------------------------
            # END OF YOUR CODE
            # ------------------------------------------------------------------
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            ...
            sy_logprob_n = tf.distributions.Categorical(logits=sy_logits_na).log_prob(value=sy_ac_na)
            # ------------------------------------------------------------------
            # END OF YOUR CODE
            # ------------------------------------------------------------------
        else:
            ...
            # START OF YOUR CODE
            # ------------------------------------------------------------------
            sy_logprob_n = tf.distributions.Normal(loc=sy_mean, scale=tf.exp(sy_logstd)).log_prob(value=sy_ac_na)
            # ------------------------------------------------------------------
            # END OF YOUR CODE
            # ------------------------------------------------------------------
        return sy_logprob_n
    ...
    def sample_trajectory(self, env, animate_this_episode):
        ...
        while True:
            ...
            # START OF YOUR CODE
            # ------------------------------------------------------------------
            f_dict = {self.sy_ob_no: [ob]}
            ac = self.sess.run(self.sy_sampled_ac, feed_dict = f_dict)
            # ------------------------------------------------------------------
            # END OF YOUR CODE
            # ------------------------------------------------------------------
            ac = ac[0]
    ...
    def sum_of_rewards(self, re_n):
        ...
        # ------------------------------------------------------------------
        q_n = []
        for path_reward in re_n:
            T = len(path_reward)

            ## Case II: Reward-to-go 
            if self.reward_to_go:
                for t in range(T):
                    path_q = 0 
                    for t_prime in range(t,T):
                        path_q += path_reward[t_prime] * (self.gamma ** (t_prime - t))
                    q_n.extend([path_q])


            ## Case I: trajectory-based PG 

            else:
                path_q = 0 
                gamma = 1 
                for i in range(T):
                    path_q = path_q + gamma * path_reward[i]
